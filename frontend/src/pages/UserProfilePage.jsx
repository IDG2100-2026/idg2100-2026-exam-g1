...
export default function UserProfilePage() {
  ...
  if (loading) return <div className="container"><LoadingSpinner message="Loading profile..." /></div>
  if (error)   return <div className="container" style={{ paddingTop: '2rem' }}><ErrorMessage message={error} /></div>
  if (!profile) return null

  const stats = lastMonthStats(profile.recentGames, id)
      ...
      <div className="card" style={styles.profileCard}>
        <div style={styles.avatarRow}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            {isOwner && (
              <label style={styles.avatarUploadLabel} title={avatarUploading ? 'Uploading...' : 'Change photo'}>
                <input
                  type="file"
          ...
          <div>
            ...
            {avatarError && <p style={{ color: 'var(--error)', fontSize: '0.8rem', marginTop: '0.25rem' }}>{avatarError}</p>}
          </div>
        </div>
          ...
          <div style={styles.sectionHeader}>
            ...
            {isOwner && !aboutEdit && (
              <button className="btn btn-secondary" style={styles.smallBtn}
                onClick={() => setAboutEdit(true)}>Edit</button>
            )}
          </div>
          {aboutEdit ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button className="btn btn-primary" style={styles.smallBtn}
                  onClick={saveAbout} disabled={aboutSaving}>
        ...
        {isOwner && (
          <div className="card" style={styles.section}>
            ...
            <form onSubmit={savePassword} style={styles.pwForm}>
              ...
              <button type="submit" className="btn btn-primary" style={styles.smallBtn} disabled={pwSaving}>
      ...
      <div className="card" style={{ ...styles.section, marginTop: '1.5rem' }}>
        <div style={styles.sectionHeader}>
          <h2 style={styles.sectionTitle}>Last 10 Games</h2>
            <Link to={`/profile/${id}/games`} style={styles.viewAll}>View all games →</Link>
        </div>
...
const styles = {
  ...
  statValue: { fontSize: '1.4rem', fontWeight: 700, color: 'var(--text-heading)' },
  ...
  success: { color: 'var(--success)', fontSize: '0.875rem', background: 'var(--success-light)', padding: '0.5rem 0.75rem', borderRadius: 'var(--radius)' },
  ...
  gameDate: { color: 'var(--text-muted)', fontSize: '0.78rem', whiteSpace: 'nowrap' },
